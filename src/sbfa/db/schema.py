...
DEFINE FIELD protocol_version ON agent_card TYPE string DEFAULT '0.3.0';
...
DEFINE INDEX idx_chunk_embedding ON rag_chunk FIELDS embedding
  HNSW DIMENSION {EMBEDDING_DIMENSION} TYPE F32 DIST COSINE EFC 500 M 16 DEFER;
...
def get_schema_sql(dimension: int = 1536) -> str:
    """Generate schema SQL with the configured embedding dimension."""
    return _SCHEMA_SQL_TEMPLATE.replace("{EMBEDDING_DIMENSION}", str(dimension))


def _strip_comments(sql: str) -> str:
    """Remove SQL line comments (--) from each line."""
    lines = []
...
async def apply_schema(db: SurrealClient, dimension: int = 1536) -> None:
    """Apply the full database schema."""
    schema_sql = get_schema_sql(dimension)
...
async def initialize_database(db: SurrealClient, dimension: int = 1536) -> None:
    """Apply schema and rebuild indexes."""
    await apply_schema(db, dimension=dimension)
